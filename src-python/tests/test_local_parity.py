...
def run_haskell(path: Path, debug_input: str | None = None) -> subprocess.CompletedProcess[str]:
  return subprocess.run(
    ([str(HASKELL_JANA), "-d", str(path)] if debug_input is not None else [str(HASKELL_JANA), str(path)]),
    cwd=ROOT,
    text=True,
    input=debug_input,
    capture_output=True,
...
class LocalParityTests(unittest.TestCase):
  ...
  def test_local_backward_debugger_stop_matches(self) -> None:
    path = ROOT / "src-python/tests/fixtures/local_backward_simple.ja"
    haskell = run_haskell(path, "f\nr\nq\n")
    python = run_python(path, "f\nr\nq\n")
    self.assertEqual(
      (python.returncode, python.stdout, python.stderr),
      (haskell.returncode, haskell.stdout, haskell.stderr),
    )
