...
class CStyleSyntaxTests(unittest.TestCase):
  def test_matrixmult_example_parses_as_c_style(self) -> None:
    source = (ROOT / "examples" / "matrixmult.ja").read_text()
    program = parse_program("examples/matrixmult.ja", source)
    canonical = format_program(program)
    reparsed = parse_program("examples/matrixmult.ja", canonical)
