...
class StructRuntimeTests(unittest.TestCase):
  ...
  def test_linked_list_example_file_runs(self) -> None:
    result = self.run_example("examples/linked-list.ja")
    self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    self.assertIn("count=3 head=3", result.stdout)
    self.assertIn("after undo: count=0 head=0 alloc=1", result.stdout)

  def test_sort_network_example_file_runs(self) -> None:
    result = self.run_example("examples/sort-network.ja")
    self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
    self.assertIn("sorted (5 swaps):", result.stdout)
    self.assertIn("{key = 1, data = 10}", result.stdout)
